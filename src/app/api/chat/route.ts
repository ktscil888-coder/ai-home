...
async function generateAIResponse(message: string, videos: any[]): Promise<string> {
  ...
  try {
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer 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`,
...
- 要体现对用户个人风格和地区特色的理解
- 🆕 **内容长度要求**: 生成的标题应控制在${lengthPreference.titleLength}字左右，脚本内容应控制在${lengthPreference.contentLength}字以上，符合用户的创作习惯`;
}
...
function generateIntelligentMockResponse(message: string, videos: any[]): string {
  ...
  // 根据用户的实际数据生成更智能的回复
  if (messageLower.includes('综合分析') || messageLower.includes('分析')) {
    if (videos.length === 0) {
...
需要我为您生成具体的内容创作方案吗？`;
  ...
  if (messageLower.includes('一键生成') || messageLower.includes('生成')) {
    if (videos.length === 0) {
...
上传您的视频数据后，我将为您生成更个性化的内容！`;
  ...
  if (messageLower.includes('追热点') || messageLower.includes('热点')) {
    return generateHotTopicContent(videos);
  }

  if (messageLower.includes('定制创作') || messageLower.includes('定制')) {
...
function generateHotTopicContent(videos: any[]): string {
  return `结合当前热点和您的创作风格，为您生成追热点内容：

🔥 **热点结合方案：**

**热点1 - 年底大扫除话题**
• 标题：《年底大扫除不用愁！家政阿姨教你3小时搞定全屋》
• 角度：结合您的专业技能，分享高效清洁方法
• 关键词：年底大扫除、家政技巧、高效清洁

**热点2 - 春节家政需求**  
• 标题：《春节家政预约火爆！提前教你这几招客户抢着要》
• 角度：展示专业服务标准，吸引潜在客户
• 关键词：春节家政、专业服务、客户好评

**热点3 - 新年新气象**
• 标题：《新年新开始！家政阿姨的2024年服务升级计划》
• 角度：展示个人成长和服务提升，增加信任感
• 关键词：新年计划、服务升级、专业成长
